...
const formatInvoice = async (invoice, tracking) => {
	try {
		...
		const invoiceInfo = {
			...
			customer: await myslqService.customers.getCustomerById(invoice[0]?.CustomerId),
			recieverId: invoice[0]?.RecieverId,
			reciever: await myslqService.recievers.getRecieverById(invoice[0]?.RecieverId),
						...
						trackingHistory: [
							...
							},
							{
								...
								createdAt: pack?.DispatchId
									? await myslqService.dispatch.getDispatchById(pack.DispatchId)
									: null,
